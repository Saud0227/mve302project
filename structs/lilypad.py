...
class Lilypad:
    ...
    def smart_edge_check(self, edge_x: float) -> None:
        ...
        l_checked = False
        cluster = self.cluster.get_top()
        if not cluster.left_connected:
            self._check_edge_connections(edge_x)
            cluster.left_connected = self.left_connected
            l_checked = True
        if not cluster.right_connected:
            if not l_checked:
                self._check_edge_connections(edge_x)
            cluster.right_connected = self.right_connected
